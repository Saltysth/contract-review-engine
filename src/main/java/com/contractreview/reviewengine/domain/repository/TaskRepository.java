...
public interface TaskRepository extends JpaRepository<Task, TaskId> {
     ...
     * 查找指定时间范围内创建的任务
     */
    @Query("SELECT t FROM Task t WHERE t.auditInfo.createdTime BETWEEN :startTime AND :endTime")
    List<Task> findByCreatedTimeBetween(@Param("startTime") LocalDateTime startTime, 
                                     @Param("endTime") LocalDateTime endTime);
    
    /**
     * 查找超时的运行中任务
     */
    @Query("SELECT t FROM Task t WHERE t.status = :status AND t.auditInfo.updatedTime < :timeoutThreshold")
     ...
     * 分页查询任务
     */
    Page<Task> findByStatusOrderByAuditInfoCreatedTimeDesc(TaskStatus status, Pageable pageable);
